...
const AdminNew = ({ user }) => {
  const {
    docs, fetching, refetch
  } = useFetchCollection('registered');

  const [eventFilter, setEventFilter] = useState('')

  return (
    <motion.div className={cx(styles['admin-page'], 'page-transition')}
      initial={{ scaleX: 0 }}
      animate={{ scaleX: 1 }}
      exit={{ scaleX: 0 }}
    >
      <div className='container'>
        ...
        <main className={styles.main}>
          ...
          <nav className={styles['filter-nav']}>
            ...
            <select required name="eventFilter" value={eventFilter} onChange={(e) => {
              setEventFilter(e.target.value)
            }} defaultChecked="All">
              <option value="">All</option>
              {Object.keys(events).filter(id => events[id].type === 'Contest').map(id => (
                <option key={id} value={id}>{events[id].title}</option>
              ))}
            </select>

            <div className={styles.count}>
              {eventFilter ? events[eventFilter].title : 'All'} : {Object.keys(docs).filter(id => docs[id].eventParticipation === eventFilter || !eventFilter).length}
            </div>
          </nav>
            ...
            <table className={styles['list-table']}>
              ...
              <tbody>
                {Object.keys(docs).filter(id => docs[id].eventParticipation === eventFilter || !eventFilter).map(id => {
                  const { TeamMembers, TeamName, TeamSize,
                    age, college, email, eventParticipation,
                    firstName, gender, lastName,
                    rollno, contact, userId } = docs[id]
                  ...
                  return (
                    <tr key={id}>
                      ...
                      <td>{email}</td>
                      {!eventFilter && (
                        <td>{events[eventParticipation] && events[eventParticipation].title}</td>
                      )}
                      <td>{id}</td>
                      <td>{userId}</td>
                    </tr>)
